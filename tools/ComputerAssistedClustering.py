...
class ComputerAssistedClustering:
    ...
    def fit(self, documents):
        ...
        if self.algorithm == 'kmeans':
            ...
            raise ValueError("Unsupported algorithm. Choose from 'kmeans', 'agglomerative', 'dbscan'.")
    ...
    def visualize_clusters(self, top_n_components=2):
        ...
        """
        pca = PCA(n_components=top_n_components, random_state=self.random_state)
        reduced_features = pca.fit_transform(self.features.toarray())
        plt.figure(figsize=(10, 7))
        scatter = plt.scatter(reduced_features[:, 0], reduced_features[:, 1], c=self.labels, cmap='viridis')
